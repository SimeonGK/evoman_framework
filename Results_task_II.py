...
#fig.suptitle("Population Evolution of Generalist Agents Using 2 Evolutionary Algorithms", fontsize=12, fontweight='bold', x=0.45, y=0.99, va='center')

###### FOR NOW ###### :
# Choose 2 test-results for comparison
comparison = ['Results_Task_II/test_8_enemies_pt1', 'Results_Task_II/test_npop_200']

for idx, group in enumerate(comparison):   # Now these are 2 results to compare, later we want these to be 2 groups of enemies
    avg_mean_total = []
    std_mean_total = []
    avg_best_total = []
    std_best_total = []
    ...
    """"
    for method in (1,2): #This is going to be Neural network vs NEAT. Now it is only one method.

        
        # Make a list of all directories with the results of 10 runs
        fitness_dir = []
        for run in range(1, runs+1):
            directory = f'method{method}_group{group}_run{run}'
    ...
    axs[idx].set_title(f'Enemy group {group}', fontsize=10)
...
#fig.tight_layout()
fig.savefig(os.path.join(plots_dir, 'Test_plot_npop_200.png'), dpi=300, bbox_inches='tight')
fig.show()
...
fig.text(0.005, 0.5, 'Fitness', va='center', ha = 'center', rotation='vertical', fontsize=10)
fig.suptitle("Population Evolution of Specialist Agents Using 2 Evolutionary Algorithms", fontsize=12, fontweight='bold', x=0.45, y=0.99, va='center')
...
# Save and show the plot
fig.tight_layout()
fig.savefig(os.path.join(save_dir, 'Test_plot_Crossover_vs_blend.png'), dpi=300, bbox_inches='tight')
fig.show()
"""